...
coef_df$type = paste(paste("RM", coef_df$running, sep = " "), paste("threshold ", coef_df$percentage, "%", sep = ""), sep = "-")


# Get coefficient estimates for models that assume that the 95th quantile of
# coral cover time series data is the upper limit.
model_list = paste("m", 1:7, sep  ="")
for (model in model_list ) {
  coef_df = rbind(coef_df, get_quantile(manta, model))
}
...
temperature_plot = ggplot(coef_df[!coef_df$interpolation == FALSE, ], aes(x = type,
  ...
  scale_y_discrete(limits = c("m7", "m6", "m5", "m4", "m3", "m2", "m1"),
                   labels = c(rep("", 7))) +
  scale_x_discrete(limits = c("quantile", "RM 5-threshold 80%" ,
                   ...
                   labels = c("quantile", "5-80%", "5-85%", "5-90%", "7-80%",
...
Figure4 = ggarrange(habitat_map + ggtitle("a") , temperature_map + ggtitle("b") , 
                    Secchi_map + ggtitle("c"), habitat_plot+ ggtitle("d"), 
                    temperature_plot + ggtitle("e"), Secchi_plot + ggtitle("f"), ncol = 3)
#ggsave(paste(path_out, "Figure4.png", sep = "/"), Figure4, width = 37, height = 45, units = "cm", dpi = 300)
...
# Load the model assuming a 5-year running mean and an 80% threshold
m = readRDS(paste(path_out, "1_5_80_TRUE.rds", sep = "/"))

# Extract and plot its conditional effects
...
Figure3c <- ggplot() +
  geom_point(data = dat, aes( x = temp_median, y = (carrying_prop ) * 100), col = "black",
             pch = 21, fill = "grey", size = 2) +
  geom_ribbon(data = cond$st_temp_median, aes( x = Temperature, ymin = lower__* 100, ymax = upper__ * 100), alpha = 0.1) +
  geom_line(data = cond$st_temp_median, aes( x = Temperature, y = estimate__ * 100), linewidth = 1) +
  xlab("Temperature (°C)") +
  ylab("Upper limit (%)") +
  theme_bw() +
...
cond$st_habitat$habitat = convert_scales("habitat", cond$st_habitat$st_habitat,
                                         mean_vals)

Figure3b <- ggplot() +
  geom_ribbon(data = cond$st_habitat, aes( x = habitat * 100, ymin = lower__* 100, ymax = upper__ * 100),  alpha = 0.1) +
  geom_line(data = cond$st_habitat, aes( x = habitat * 100, y = estimate__ * 100), linewidth = 1) +
  geom_point(data = dat, aes( x = per_suitable * 100, y = (carrying_prop* 100)), fill = "grey", col = "black",
             pch = 21,  size = 2) +
...
cond$st_Secchi$Secchi = convert_scales("Secchi", cond$st_Secchi$st_Secchi,
                                       mean_vals)

Figure3d <- ggplot() +
  geom_ribbon(data = cond$st_Secchi, aes( x = Secchi, ymin = lower__* 100, ymax = upper__ * 100),  alpha = 0.1) +
  geom_line(data = cond$st_Secchi, aes( x = Secchi, y = estimate__ * 100), linewidth = 1) +
  geom_point(data = dat, aes( x = Secchi_median, y = (carrying_prop* 100)), fill = "grey", col = "black",
             pch = 21,  size = 2) +
  xlab("Secchi depth (m)") +
  ylab("Upper limit (%)") +
...
post <- as.data.frame(as_draws_df(m)[ , 2:4])
colnames(post) <- c( "habitat", "temp", "Secchi")
...
Figure3a <- ggplot() +
  ...
  geom_violin(data = post, aes(x = 0, y = habitat), fill = "grey" , alpha = 0.2) +
  geom_linerange(data = post, aes(x = 0, ymin = quantile(habitat, 0.025), ymax = quantile(habitat, 0.975)), 
                 size = 1.1) +
  geom_point(data = post, aes(x = 0, y = quantile(habitat, 0.5)), 
             size = 5) +
  ...
  xlab("") +
  theme_bw() +
  scale_x_continuous(breaks = 0:2, labels = c("Hard substrate", "Temperature", "Secchi depth"))+
  theme(text = element_text(size = 14 ), axis.text.x = element_text(size = 14)) +
  ggtitle("a") +
  ylab("Coefficient estimates")


Figure3 <- Figure3a /  (Figure3b + Figure3c + ylab("") + Figure3d + ylab("")) 
Figure3
#ggsave(paste(path_out, "Figure3.png", sep = "/"), Figure3, width = 35, height = 30, units = "cm", dpi = 300)
...
# variables (for the text in the results section). The relationships are nonlinear,
# therefore, we need to get a range of change across the observed environmental
# values.

# Change in upper limits with a 10% change in hard substrate availability
preds_habitat = data.frame("habitat" = c(0, 0.1, 0.9, 1))
preds_habitat$st_temp_median = cond$st_habitat$st_temp_median[1]
preds_habitat$st_Secchi = cond$st_habitat$st_Secchi[1]
preds_habitat$st_habitat = (preds_habitat$habitat - mean_vals[mean_vals$variable == "habitat", ]$mean)/
  mean_vals[mean_vals$variable == "habitat", ]$sd

# Change in upper limits with a 1 degree C change in temperature
preds_temp = data.frame("temp" = c(24, 25, 26, 27))
preds_temp$st_habitat = cond$st_temp_median$st_habitat[1]
preds_temp$st_Secchi = cond$st_temp_median$st_Secchi[1]
preds_temp$st_temp_median = (preds_temp$temp - mean_vals[mean_vals$variable == "temperature", ]$mean)/
  mean_vals[mean_vals$variable == "temperature", ]$sd
cbind(preds_temp, as.data.frame(fitted(m, preds_temp, re_formula = NA, type = "response")))

# Change in upper limits with a 1m  change in water clarity
preds_Secchi = data.frame("Secchi" = c(13, 14, 27, 28))
preds_Secchi$st_temp_median = cond$st_Secchi$st_temp_median[1]
preds_Secchi$st_habitat = cond$st_Secchi$st_habitat[1]
...
cbind(preds_Secchi, as.data.frame(fitted(m, preds_Secchi, re_formula = NA, type = "response")))


# Fit a model that has an intercept and the random effect of reef only, for comparison
prior <- get_prior(carrying_prop ~ 1 +
                     (1| Reef), 
                   data = dat,
                   family = "beta")

m0 <- brm(carrying_prop ~ 1 + 
            (1| Reef), 
          data = dat,
          family = "beta", chains = 3, cores = 3, 
          prior = prior, iter = 20000, thin = 5,
          control = list(adapt_delta = 0.99, max_treedepth = 17),
          save_pars = save_pars(all = TRUE), file = paste(path_out, "m0_5_80.rds", sep = "/"))

# Extract the R2 of the null model
bayes_R2(m0)
